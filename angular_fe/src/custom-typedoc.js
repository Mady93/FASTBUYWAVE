...
(function () {
  ...
  var CATEGORY_ICONS = {
    ...
    'Models':         { gradient: 'linear-gradient(135deg, #f59e0b, #ef4444)' },
    'Enumerations':   { gradient: 'linear-gradient(135deg, #ec4899, #f43f5e)' },
    'Pipes':          { gradient: 'linear-gradient(135deg, #f97316, #ef4444)' },
    'Directives':     { gradient: 'linear-gradient(135deg, #ec4899, #a855f7)' },
    'Utils':          { gradient: 'linear-gradient(135deg, #6b7280, #9ca3af)' },
    'Constants':      { gradient: 'linear-gradient(135deg, #fbbf24, #f59e0b)' },
    'Dialogs':        { gradient: 'linear-gradient(135deg, #8b5cf6, #c084fc)' },
    'Icons':          { gradient: 'linear-gradient(135deg, #ec4899, #f97316)' },
    'Environments':   { gradient: 'linear-gradient(135deg, #06b6d4, #3b82f6)' },
    'Configuration':  { gradient: 'linear-gradient(135deg, #64748b, #475569)' },
    'Authentication': { gradient: 'linear-gradient(135deg, #3b82f6, #1e40af)' },
    'Geocoding':      { gradient: 'linear-gradient(135deg, #10b981, #14b8a6)' },
    'Routing':        { gradient: 'linear-gradient(135deg, #f97316, #ef4444)' },
    'Types':          { gradient: 'linear-gradient(135deg, #a855f7, #d946ef)' },
    'Guards':         { gradient: 'linear-gradient(135deg, #3b82f6, #06b6d4)' }
  };
...
function getCategoryFromLink(link) {
  ...
  // AUTHENTICATION (Interceptor, Guard, Auth services)
  if (text.includes('interceptor') || text.includes('auth') || name.includes('.guard')) {
  ...
  // INTERFACES (DTO, Request, Response, ApiResponse, State)
  if (text.includes('dto') || text.includes('response') || text.includes('request') ||
      name.includes('.interface') || name.includes('.dto') ||
      text === 'state' || text === 'apiresponse' || text === 'apiresponsedata' ||
      text === 'appointmentproposaldto' || text === 'commenttreedto' ||
      text === 'likerequestdto' || text === 'likeuserdto' ||
      text === 'countryresponse' || text === 'countrystatesresponse') {
    return 'Interfaces';
  }

  // MODELS
  if (text.includes('carousel') || text.includes('footer') || text.includes('navbar') ||
      text === 'carouselitems' || text === 'footerlinkitem' || 
      text === 'footerpaymentmethoditem' || text === 'navbaritem') {
    return 'Models';
  }
  ...
  if (name.includes('.enum')) return 'Enumerations';
  if (name.includes('.guard')) return 'Authentication';
  if (name.includes('.type')) return 'Types';
  if (name.includes('.config')) return 'Configuration';
  if (name.includes('.route')) return 'Routing';
  ...
  if (kindIcon) {
    ...
    if (label.includes('enum')) return 'Enumerations';
    if (label.includes('function') || label.includes('method') || label.includes('call')) return 'Utils';
    if (label.includes('type')) return 'Types';
    if (label.includes('variable') || label.includes('const')) return 'Constants';
    if (label.includes('guard')) return 'Authentication';
  }
